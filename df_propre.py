...
class Recup_Donneees_VP:
    def __init__(self):
        ...
        # self.Recuperation_Des_Fichiers_en_ligne('https://dati.mit.gov.it/catalog/dataset/dataset-parco-circolante-dei-veicoli','/Italie_data/','.zip')
    ...
    # '\Italie_data\metadata_parco_circolante.json'
    ...
    def Recuperation_Fichier_France(self):
        # print (p+'/France_data/*.xlsx')
        # self.fichier_existe(p + '/France_data/*.xlsx')
        self.fichier_existe ('/France_data/*.xlsx')
        # print (self.fichiersExistes)

        if not self.fichiersExistes and self.connexion:
            print('Récupération du fichier en ligne')
            Parc_VP_France = 'https://www.statistiques.developpement-durable.gouv.fr/sites/default/files/2022-10/parc_vp_commune_2022.xlsx'
            df_fr = pd.read_excel(Parc_VP_France)
            df_fr.to_excel(p + r'/France_data/parc_vp_commune_2022.xlsx')
            ...
            print ("Utilisation du fichier déjà téléchargé")
            df_fr_original = pd.read_excel(p + r'/France_data/parc_vp_commune_2022.xlsx', skiprows=3, header=0)
            self.df_fr_original = df_fr_original

    # 'https://dati.mit.gov.it/catalog/dataset/dataset-parco-circolante-dei-veicoli'
    # '/Italie_data/'
    # '.zip'

    # df_fr_original = self.df_fr_original

    def renommage(self):
        df_fr_original = pd.read_excel(p + r'/France_data/parc_vp_commune_2022.xlsx', skiprows=3, header=0)
        df_fr_original = df_fr_original.rename(columns={ 'Code commune de résidence': 'code_commune_insee_residence', 'Commune de résidence': 'commune_de_residence', 
    ...
    def df_propre_final_fr(self):
        ...
        df_ccicp = pd.read_csv(p+r'/France_data/correspondance-code-insee-code-postal.csv',sep=';')

        print ('df_ccicp : ',df_ccicp.shape,'\n')

        # df_ccicp_geoloc_propre = df_ccicp[df_ccicp['Statut'].isin(["['Préfecture']", "['Préfecture de région']",'["Capitale d\'état"]'])]
        df_ccicp['Code Postal'] = df_ccicp['Code Postal'].astype(str)
        df_ccicp['Code Postal'] = df_ccicp['Code Postal'].str[:5]
        df_ccicp_geoloc_propre = df_ccicp
        ...
        parc_vp_propre_geoloc.to_csv(p + r'/France_data/parc_vp_propre_geoloc.csv',encoding="utf-8")
